...
/* defines */
  ...
  #define TRACK // #define TRACK    {if(TRACKING){char command[300]; wsprintf( command, "c:\\program files\\insecta\\insecta2.exe  %s  -Goto %d  %d ", "insecta.ins" , __LINE__ , __LINE__ );  WinExec( command , NULL );}};
  #define TRACK // #define BREAK  {if(TRACKING){char command[300]; wsprintf( command, "c:\\program files\\insecta\\insecta2.exe  %s  -Goto %d  %d ", "insecta.ins" , __LINE__ , __LINE__ );  WinExec( command , NULL ); MES;
/* prototypes */ 
...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) //  ... main entry point for the windows program;
  ...
  *vfc99_directory = NULL;
  ...
  { // /  ;
    ...
    { // /  ;
      MessageBox(MainWindow,"The time limit on this evaluation copy has expired - VFC will only run in DEMO mode now.  Please contact FlowLynx to purchase an unrestricted license.","Time Limit Expired",0); // /  ;
      MAX_DEMO_OBJECTS=NUM_OBJECTS_LIMIT ; // /  ;
    } // /  ;
  #endif
...
int APIENTRY __WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) //  ... main entry point for the windows program;
  ...
  { // /  ;
    ...
    { // /  ;
      MessageBox(MainWindow,"The time limit on this evaluation copy has expired - FlowCoder will only run in DEMO mode now.  Please contact FlowLynx at www.flowlynx.com to purchase a license.","Time Limit E
      MAX_DEMO_OBJECTS=NUM_OBJECTS_LIMIT ; // /  ;
    } // /  ;
  #endif
